...
对 `index.yaml` 中 8 个专辑**串行**执行（Playwright 单浏览器不支持真正并行）：
...
2. `browser_evaluate` 提取文章列表：
   ...
   ```
3. 将结果覆盖写入 `index.yaml` 对应专辑的 `articles`（**保留已有的 `date` 字段**，只更新 title/url）

4. 全部专辑更新完成后，从所有文章 URL 提取 mid，**按 mid 降序取前 8 篇**，逐篇访问文章页抓取发布日期：
   ```javascript
   () => document.querySelector('#publish_time')?.innerText?.trim()
   ```
   将日期（格式 `YYYY-MM-DD`）写入 index.yaml 对应文章的 `date` 字段。

5. 更新 `meta.updated` 为今天日期，告知用户：
...
- 正确做法：从 index.yaml 的所有文章 URL 中提取 `mid` 参数，mid 值越大表示发布越晚，取 mid 最大的前 8 篇

**步骤**：

1. 读取 `index.yaml`，遍历所有专辑的所有文章，从每篇 URL 中用正则 `mid=(\d+)` 提取 mid 值
2. 按 mid 降序排列，取前 8 篇（mid 最大 = 最新）
3. **检查这 8 篇在 index.yaml 中是否已有 `date` 字段**：
   - **有** → 直接读取，**跳过 Playwright**，0 网络请求
   - **无**（首次初始化或极少数情况）→ 逐篇访问文章页获取日期，写回 index.yaml
4. 展示结果，标注哪些是今天/昨天：
