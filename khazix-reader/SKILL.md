...
Read `index.yaml`，检查 `meta.updated` 字段：
- 如果等于今天日期 → 跳过，直接路由
- 如果不是今天 → **静默执行流程 4（刷新索引）**，完成后再路由

刷新完成后告知用户一句：`📡 索引已自动更新（YYYY-MM-DD）`，然后继续处理原始请求。
...
**触发**：用户从流程 1、2、5 中选择了某篇文章，或粘贴了 URL。
...
**方案 B（来自流程 2、5 的文章）**：流程 2/5 已取得含 chksm 的 URL，直接用。
...
**触发**：用户说「刷新」、「更新索引」、「同步最新文章」，或由入口判断自动触发。
...
3. 对这 8 篇依次 `browser_navigate` + `browser_evaluate` 获取发布日期：
   ```javascript
