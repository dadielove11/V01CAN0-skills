...
**注意**：不能用搜狗关键词搜索——那样只会返回其他账号提及卡兹克名字的文章，不是卡兹克本人账号发布的。正确做法是直接访问各专辑页，专辑页文章按时间倒序排列，第一篇就是最新的。

**步骤**：

1. 读取 `index.yaml` 中所有 8 个专辑的 `url`
2. 对每个专辑依次 `browser_navigate` 打开专辑页，`browser_evaluate` 提取前 2 篇：
   ```javascript
   () => {
     const items = document.querySelectorAll('.js_album_item');
   ...
   ```
3. 汇总所有专辑结果（最多 16 篇），按 `data-time`（Unix 时间戳降序）排序，取前 5-8 篇
   - 若 `data-time` 为空，对每篇文章单独访问提取 `#publish_time` 补充日期
