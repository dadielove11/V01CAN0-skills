...
**方案 B（来自流程 2、5 的文章）**：流程 2/5 已取得 Sogou 跳转链接，直接用。
...
**触发**：用户说「最近发了什么」「昨天发了什么」「今天有没有新文章」「最新几篇」。

**步骤**：

1. 用 `browser_navigate` 打开搜狗账号搜索（不带关键词，结果按时间倒序）：
   ```
   https://weixin.sogou.com/weixin?type=2&query=数字生命卡兹克&ie=utf8
   ```

2. `browser_snapshot` 获取页面，筛选来源为「数字生命卡兹克」的条目，提取标题、日期、摘要、Sogou 跳转链接，取前 5-8 条
